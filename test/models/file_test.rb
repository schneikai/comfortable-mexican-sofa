...
class CmsFileTest < ActiveSupport::TestCase
  ...
  def test_scope_images
    ...
    assert_equal 2, Comfy::Cms::File.not_images.count
  end

  def test_scope_not_page_file
    file = comfy_cms_files(:default)
    assert file.block.blank?
    assert_equal 2, Comfy::Cms::File.not_page_file.count
  ...
  def test_content_caching
    ...
    assert_equal file.content_cache, file.render

    file.update_columns(:content_cache => 'Old Content')
    refute_equal file.content_cache, file.render

    file.clear_content_cache!
    assert_equal file.content_cache, file.render
  end
  ...
  def test_content_cache_clear_on_save
    ...
    file.update_columns(:content_cache => old_content)
